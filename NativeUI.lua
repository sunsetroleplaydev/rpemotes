...
function UIMenu.New(Title, Subtitle, X, Y, TxtDictionary, TxtName)
    ...
    if Subtitle ~= "" and Subtitle ~= nil then
        ...
        _UIMenu.Subtitle.Text = UIResText.New(string.upper('~HUD_COLOUR_FREEMODE~' .. Subtitle), 8 + _UIMenu.Position.X, 110 + _UIMenu.Position.Y, 0.35, 245, 245, 245, 255, 0)
        _UIMenu.Subtitle.BackupText = string.upper('~HUD_COLOUR_FREEMODE~' .. Subtitle)
        _UIMenu.Subtitle.Formatted = false
        if string.starts(Subtitle, "~") then
            _UIMenu.PageCounter.PreText = string.sub(string.upper('~HUD_COLOUR_FREEMODE~' .. Subtitle), 1, 3)
        end
...
function MenuPool:AddSubMenu(Menu, Text, Description, Label, KeepPosition, KeepBanner)
    if Menu() == "UIMenu" then
